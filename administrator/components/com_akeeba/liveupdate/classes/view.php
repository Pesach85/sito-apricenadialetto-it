...
class LiveUpdateView extends JView
{
	public function display($tpl = null)
	{
		...
		$requeryURL = 'index.php?option='.$input->getCmd('option','').'&view='.$input->getCmd('view','liveupdate').'&force=1';
		...
		JToolBarHelper::back($msg, 'index.php?option='.$input->getCmd('option',''));
		
		switch($input->getCmd('task','default'))
		{
			case 'startupdate':
				$this->setLayout('startupdate');
				$this->assign('url','index.php?option='.$input->getCmd('option','').'&view='.$input->getCmd('view','liveupdate').'&task=download');
				break;
			...
			case 'nagscreen':
				...
				$this->assign('runUpdateURL','index.php?option='.$input->getCmd('option','').'&view='.$input->getCmd('view','liveupdate').'&task=startupdate&skipnag=1');
				break;
			...
			default:
				...
				$this->assign('runUpdateURL','index.php?option='.$input->getCmd('option','').'&view='.$input->getCmd('view','liveupdate').'&task=startupdate');
