...
REMOTE_PATH = "/home/w19158/public_html/administrator/components/com_installer/models/fields/folder.php"
...
def main() -> int:
    ...
    if "getExtensionGroupes" not in content:
        print("NO_CHANGE")
    else:
        backup_path = REMOTE_PATH + ".bak_groupes_" + datetime.now().strftime("%Y%m%d_%H%M%S")
        sftp.posix_rename(REMOTE_PATH, backup_path)
        print("BACKUP_OK", backup_path)

        fixed = content.replace("getExtensionGroupes", "getExtensionGroups")
