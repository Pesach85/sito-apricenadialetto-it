...
def main() -> int:
    parser = argparse.ArgumentParser(description="Overlay pulito Joomla 4 su produzione")
    parser.add_argument("--apply", action="store_true", help="Esegue overlay reale")
    ...
    production_root = cfg["remotePath"].rstrip("/")

    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    work_root = f"/home/{username}/upgrade_work/j4_prod_{ts}"
    ...
    print("J4_PRODUCTION_OVERLAY_PLAN")
    print(json.dumps(plan, ensure_ascii=False, indent=2))

    preflight_cmds = {
        "production_exists": f"test -d {shlex.quote(production_root)} && echo OK || echo FAIL",
        ...
        "disk_space": f"df -h {shlex.quote(posixpath.dirname(production_root))} | tail -n 1",
    }
    ...
    if preflight.get("production_exists") != "OK":
        ssh.close()
        raise RuntimeError("Production root non trovato")
    ...
    ssh_exec(ssh, "chmod u+w " + shlex.quote(posixpath.join(production_root, "configuration.php")))

    unzip_cmd = "unzip -o " + shlex.quote(package_file) + " -d " + shlex.quote(production_root)
    ...
    verify_version_cmd = (
        ...
        + shlex.quote(
            "$f='"
            + posixpath.join(production_root, "administrator/manifests/files/joomla.xml")
            + "';"
    ...
    cache_clear_cmd = (
        "find "
        + shlex.quote(production_root + "/cache")
        + " -type f ! -name index.html -delete; "
        + "find "
        + shlex.quote(production_root + "/tmp")
        + " -type f ! -name index.html -delete"
    )
    ...
    ssh_exec(ssh, "chmod 444 " + shlex.quote(posixpath.join(production_root, "configuration.php")))

    ssh.close()

    print("J4_PRODUCTION_OVERLAY_OK")
    print("detected_version=", detected_version)
    print("note= Rieseguire verifica forense versione file core")
    return 0
